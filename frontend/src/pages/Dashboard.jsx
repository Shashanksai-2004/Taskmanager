...
const Dashboard = () => {
  ...
  const stats = useMemo(() => ({
    ...
    pct: tasks.length ? Math.round((tasks.filter((t) => t.stage === 'Done').length / tasks.length) * 100) : 0,
  }), [tasks]);
  ...
  const handleDragEnd = async ({ destination, source, draggableId }) => {
    ...
    setTasks((p) => p.map((t) => (t._id === draggableId ? { ...t, stage: newStage } : t)));
    try {
      await taskService.updateTask(draggableId, { stage: newStage });
      toast.success(`Moved to ${newStage}`);
    } catch {
      setTasks((p) => p.map((t) => (t._id === draggableId ? { ...t, stage: source.droppableId } : t)));
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
            ...
            {!loading && stats.total > 0 && (
              ...
              >
                ...
                <span className="text-sm font-bold text-slate-300">{stats.pct}%</span>
              </motion.div>
            )}
          </div>
        </motion.div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <div className="relative ml-auto">
            <button
              ...
              className="flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-xl border border-white/8 bg-white/4 text-slate-400 hover:text-slate-300 hover:bg-white/8 transition-all"
            >
            ...
            <AnimatePresence>
              {sortOpen && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2 w-44 rounded-xl border border-white/10 shadow-2xl z-30 overflow-hidden"
                  style={{ background: 'rgba(10,14,28,0.98)', backdropFilter: 'blur(16px)' }}
                >
                  {SORT_OPTIONS.map((o) => (
                    <button
                      key={o.value}
                      ...
                      style={{ color: sortBy === o.value ? '#818cf8' : '#94a3b8' }}
          ...
          {search && (
            <motion.button
              ...
              onClick={() => setSearch('')}
              className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300 px-2.5 py-1.5 rounded-lg hover:bg-white/6 border border-white/8 transition-all"
            >
              <X size={11} />
              Clear search
            </motion.button>
        ...
        {!loading && search && processedTasks.length === 0 && (
          ...
          >
            ...
            <h3 className="text-slate-300 font-semibold text-lg">No tasks found</h3>
